...
def _clean_vcf(output_prefix, fasta_ref, input_vcf, keep_all=False):
    ...
    for record in input_vcf_f:
        if not keep_all and 'PASS' not in record.filter.keys():
            continue
    ...
    if len(variants) > 0:
        ...
        for variant_record in variants:
            ...
            record_str = '\t'.join(tab_sep_str[:8] + ['GT', '0/0', '0/1'])
            # TODO: What with multiallelic variants?
